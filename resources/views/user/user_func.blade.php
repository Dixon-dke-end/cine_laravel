...
<body>
    ...
    <nav class="navbar">
        <div class="nav-container">
            <a href="{{ route('user.index') }}" class="logo">CINEVEL</a>
            ...
            <div class="user-section">
                ...
                @else
                    @if(Auth::user()->role === 'admin')
                        <a href="{{ route('movies.index') }}" class="btn btn-admin" title="Ver vista de administrador">
                            🔧 Admin
                        </a>
                    @endif
                    <div class="user-info">
    ...
    <!-- Hero Section -->
    <section class="hero">
        <div class="hero-bg" style="background-image: url('{{ asset('storage/' . $movie->ruta_imagen) }}');"></div>
    ...
    <div class="container">
            ...
            <div class="details-section">
                ...
                <div class="meta-grid">
                    <div class="meta-item">
                        ...
                        <span class="badge">{{ $movie->age_suggest }}</span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">Duración</span>
                        <span class="meta-value">{{ $movie->duracion }} min</span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">Género</span>
                        <span class="badge">{{ $movie->genero }}</span>
                    </div>
                    @if($movie->año)
                    <div class="meta-item">
        ...
        <section class="schedule-section" id="scheduleSection">
                    ...
                    <div class="dates-container">
                        <div class="dates-track" id="datesTrack">
                            @foreach($fechasCalendario as $index => $fecha)
                                ...
                                @endphp
                                <div class="date-card {{ $esActivo ? 'active' : '' }}" 
                                     data-fecha="{{ $fecha }}"
                                     onclick="selectDay(this)">
                                    <div class="date-day">{{ $carbon->locale('es')->isoFormat('ddd') }}</div>
                                    <div class="date-number">{{ $carbon->format('d') }}</div>
                                    <div class="date-month">{{ $carbon->locale('es')->isoFormat('MMM') }}</div>
                                </div>
                            @endforeach
                        </div>
                    </div>
            ...
            <div id="cinemaList">
                ...
                @if($funcionesDelDia->count() > 0)
                    @foreach($funcionesDelDia as $salaNombre => $funcionesSala)
                        <div class="cinema-group">
                            <div class="cinema-name">🎭 {{ htmlspecialchars($salaNombre) }}</div>
                            <div class="showtimes-grid">
                                @foreach($funcionesSala as $funcion)
                                    @php
                                        $hora = \Carbon\Carbon::parse($funcion->hora)->format('H:i');
                                    @endphp
                                    <button class="showtime-card" onclick="reservarFuncion({{ $funcion->id }})">
                                        <div class="showtime-time">{{ $hora }}</div>
                                        <div class="showtime-room">Sala {{ $funcion->sala_id }}</div>
                                    </button>
                                @endforeach
                            </div>
                        </div>
    ...
    <div class="modal" id="trailerModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title">Tráiler - {{ $movie->titulo }}</h3>
