            ...
            <div class="form-container">
                <form action="{{ route('promociones.update', $promocion->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="titulo" id="titulo" class="form-control" value="{{ old('titulo', $promocion->titulo) }}" required>
                        @error('titulo') <div class="text-danger">{{ $message }}</div> @enderror
                    </div>

                    <div class="form-group">
                        <label for="descripcion" class="form-label">Descripción</label>
                        <textarea name="descripcion" id="descripcion" class="form-control" rows="4" required>{{ old('descripcion', $promocion->descripcion) }}</textarea>
                        @error('descripcion') <div class="text-danger">{{ $message }}</div> @enderror
                    </div>

                    <div class="form-group">
                        <label for="codigo" class="form-label">Código (Opcional)</label>
                        <input type="text" name="codigo" id="codigo" class="form-control" value="{{ old('codigo', $promocion->codigo) }}">
                        @error('codigo') <div class="text-danger">{{ $message }}</div> @enderror
                    </div>

                    <div class="form-group">
                        <label for="descuento" class="form-label">Descuento (%)</label>
                        <input type="number" step="0.01" name="descuento" id="descuento" class="form-control" value="{{ old('descuento', $promocion->descuento) }}">
                        @error('descuento') <div class="text-danger">{{ $message }}</div> @enderror
                    </div>

                    <div class="form-group">
                        <label for="fecha_inicio" class="form-label">Fecha Inicio</label>
                        <input type="date" name="fecha_inicio" id="fecha_inicio" class="form-control" value="{{ old('fecha_inicio', $promocion->fecha_inicio) }}" required>
                        @error('fecha_inicio') <div class="text-danger">{{ $message }}</div> @enderror
                    </div>

                    <div class="form-group">
                        <label for="fecha_fin" class="form-label">Fecha Fin</label>
                        <input type="date" name="fecha_fin" id="fecha_fin" class="form-control" value="{{ old('fecha_fin', $promocion->fecha_fin) }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <select name="tipo" id="tipo" class="form-control" required>
                            <option value="general" {{ old('tipo', $promocion->tipo) == 'general' ? 'selected' : '' }}>General</option>
                            <option value="descuento" {{ old('tipo', $promocion->tipo) == 'descuento' ? 'selected' : '' }}>Descuento</option>
                            <option value="boleteria" {{ old('tipo', $promocion->tipo) == 'boleteria' ? 'selected' : '' }}>Boleteria</option>
                            <option value="promociones" {{ old('tipo', $promocion->tipo) == 'promociones' ? 'selected' : '' }}>Promociones</option>
                            <option value="confiteria" {{ old('tipo', $promocion->tipo) == 'confiteria' ? 'selected' : '' }}>Confiteria</option>
                            <option value="combos" {{ old('tipo', $promocion->tipo) == 'combos' ? 'selected' : '' }}>Combos</option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="activo" id="activo" class="form-control" required>
                            <option value="1" {{ old('activo', $promocion->activo) == 1 ? 'selected' : '' }}>Activo</option>
                            <option value="0" {{ old('activo', $promocion->activo) == 0 ? 'selected' : '' }}>Inactivo</option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        @if($promocion->imagen)
                            <img src="{{ asset('storage/' . $promocion->imagen) }}" alt="Imagen actual" class="current-image">
                        @endif
